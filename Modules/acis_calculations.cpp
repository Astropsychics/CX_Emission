...
int acis_calculations(double energy_start, double energy_end, double energy_step, int comet_number){
   ...
   //outputs re-scaled ACIS spectrum
   string output_name_Chandra = "../Results/" + comet_name + "/ACIS_CX_spectrum_"+ comet_name + ".dat";
   ofstream output_file_Chandra(output_name_Chandra.c_str());
   for( int i=0; i<energy_row; i++ ){
       int z = 0;
       while ( input_area[z][1] <= input_energy[i] ) z++;

       output_file_Chandra << scientific << input_CX[i][0] << " " << input_CX[i][1] * input_area[z][2] << endl;
   }
